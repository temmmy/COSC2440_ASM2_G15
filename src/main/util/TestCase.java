...
//        testSearchIntersectsMultipleNodes(map);//Test case 24
    ...
    private static void testInsertToEmptyTree() {
        Map2D map = Map2D.getInstance();
        boolean result = map.insert(new Place(new Point(500, 500), "Cafe Central", new ServiceType[]{ServiceType.CAFE}));
    ...
    private static void testRemoveFromEmptyTree() {
        Map2D map = Map2D.getInstance();
        boolean result = map.remove(new Place(new Point(1000, 1000), "Nonexistent Place", new ServiceType[]{ServiceType.HOTEL}));
    ...
    private static void testInsertToFullTree() {
        ...
        boolean result = map.insert(new Place(new Point(200, 200), "Extra Place", new ServiceType[]{ServiceType.GYM}));
    ...
    private static void testRemoveFromFullNode() {
        ...
        map.display();
        System.out.println("After inserting: ");
        boolean result = map.remove(new Place(new Point(300, 400), "Place 3", new ServiceType[]{ServiceType.RESTAURANT}));
    ...
    private static void testInsertOutsideBoundary() {;
        Map2D map = Map2D.getInstance();
        boolean result = map.insert(new Place(new Point(Map2D.MAP_SIDE + 100, Map2D.MAP_SIDE + 100), "Out of Bounds Place", new ServiceType[]{ServiceType.ATM}));
    ...
    private static void testFillServiceListToMaxCapacity() {
        ...
        for (int i = 0; i < Place.MAX_SERVICES; i++) {
            place.addService(ServiceType.values()[i % ServiceType.values().length]); // Adding unique services assuming enough types
            count++;
        }
        if (count == 9)
            check = true;
    ...
    private static void testAddServiceToFullList() {
        Place place = new Place(new Point(0, 0), "Popular Cafe", new ServiceType[]{ServiceType.ATM,ServiceType.HOSPITAL,ServiceType.RESTAURANT,ServiceType.CAFE,
            ServiceType.HOTEL,ServiceType.SCHOOL,ServiceType.PARK,ServiceType.CINEMA,ServiceType.GYM,ServiceType.STORE});
        System.out.println(place);
